...
def signup_for_activity(activity_name: str, email: str):
    ...
    if email in activity["participants"]:
        raise HTTPException(
            status_code=400, detail=f"Student {email} is already signed up for {activity_name}")
...
def unregister_from_activity(activity_name: str, email: str):
    ...
    if email not in activity["participants"]:
        raise HTTPException(
            status_code=400, detail=f"Student {email} is not signed up for {activity_name}")
